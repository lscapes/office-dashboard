...
// Runtime state (separate from SS.projects so edits don't mutate source)
let photoState = null;
let photoFilter = 'all';
let pmFilter = 'all';

function initPhotoState() {
  if (!photoState) photoState = JSON.parse(JSON.stringify(SS.projects));
}
...
function statusPill(pct) {
  return pct === 100 ? 'pill-green' : pct > 0 ? 'pill-yellow' : 'pill-red';
}
...
function toggleShot(id, key) {
  ...
  if (el) el.outerHTML = renderPhotoCard(p);
  // Re-open if it was open
  // (card closes on re-render — acceptable tradeoff for simplicity)
  updatePhotoStats();
}

function updatePhotoStats() {
  // lightweight — just re-render full page to keep stats in sync
  renderPhotoPage();
}
...
function toggleJessa(id, field) {
  ...
  if (field === 'needed') {
    ...
    if (!p.jessaNeeded) p.jessaDone = false; // reset done if no longer needed
  } else {
